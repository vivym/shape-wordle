...
const wordleAlgorithm = (drawnWords, word, options, regionID, regions) => {
  const { width: canvasWidth, height: canvasHeight } = options
  const { center, dist } = regions[regionID]
  let cnt = 0
  let lastCollidedItem = null
  do {
    ++cnt

    const startPoint = center[word.epID].pos
    for (let i = 0; i < 5; i ++) {
    ...
    if (!isInShapeWord(word, options, regionID, regions)) continue
...
const isInShapeWord = (word, { width: canvasWidth, height: canvasHeight, regionMask }, regionID, regions) => {
  const ratio = Math.abs(0.7 - word.weight) > eps ? 0.9 : 1.0
  const p = getCornerPoints(word, ratio)

  const ok = 
    isInShapePoint(p[0], canvasWidth, canvasHeight, regionMask, regionID) &&
    isInShapePoint(p[1], canvasWidth, canvasHeight, regionMask, regionID) &&
    isInShapePoint(p[2], canvasWidth, canvasHeight, regionMask, regionID) &&
    isInShapePoint(p[3], canvasWidth, canvasHeight, regionMask, regionID)
  if (!ok) return false
...
const isInShapePoint = (point, canvasWidth, canvasHeight, regionMask, regionID) => {
  let [x, y] = point
  x = Math.floor(x)
  y = Math.floor(y)
  if (
    ...
    return regionMask[y][x] === regionID
  } else {
    return false
  }
}
